...
void SmlMouseFull::init() {
    ...
    // 1. Initialise Joints
    initialiseJoints();
    print_debug("[ok] : joints initialisation (SmlMouseFull.cc)");

    // 2. Initialise Muscles
    initialiseMuscles();
    print_debug("[ok] : muscles initialisation (SmlMouseFull.cc)");

    // 3. Initialise Sensors
    initialiseSensors();
    print_debug("[ok] : sensors initialisation (SmlMouseFull.cc)");

    // 4. Initialise Data Read
    intiailiseLogData();
    print_debug("[ok] : Data logging initialisation (SmlMouseFull.cc)");

}

// Initialise constant
void SmlMouseFull::initialiseConstant()
{
    dt = double(time_step) / 1000.0;
}

// 1. Initialise Joints
void SmlMouseFull::initialiseJoints()
{
...
// 2. Initialise Muscles
void SmlMouseFull::initialiseMuscles()
{
...
// 3. Initialise Sensors
void SmlMouseFull::initialiseSensors()
{
...
// 4. Initialise Data Read
void SmlMouseFull::intiailiseLogData() {
    // Open file to log data
    logData.open("../../../results/LocomotionData.txt");
    logData << "Time \t";
...
double SmlMouseFull::getTime() {
    return eventManager->get<double>(STATES::TIME);
}

//compute l_MTC
void SmlMouseFull::step_MTU_to_TORQUE(double dt) {
...
void SmlMouseFull::step_JOINT(double dt) {
    for (auto&  joint : joints) {
        joint->step(dt);
    }
...
void SmlMouseFull::step_TORQUE_to_JOINT() {
...
void SmlMouseFull::step_SENSORS() {
...
void SmlMouseFull::step_SPINE_TO_MTU(double dt) {

}

void SmlMouseFull::initialiseSpinalControl() {

}

int SmlMouseFull::step() {
    //Timer timer;
    //timer.Start();
    InputUpdate();
    ...
    logData << muscles[0]->musclejoints[0]->getMomentArm() << "\t";
    logData << muscles[1]->musclejoints[0]->getMomentArm() << "\t";
    logData << muscles[2]->musclejoints[0]->getMomentArm() << "\t";
    logData << muscles[2]->musclejoints[1]->getMomentArm() << "\t";
    logData << muscles[3]->musclejoints[0]->getMomentArm() << "\t";
    logData << muscles[4]->musclejoints[0]->getMomentArm() << "\t";
    logData << muscles[5]->musclejoints[0]->getMomentArm() << "\t";
    logData << muscles[6]->musclejoints[0]->getMomentArm() << "\t";
    logData << muscles[7]->musclejoints[0]->getMomentArm() << "\t";
    logData << muscles[7]->musclejoints[1]->getMomentArm() << "\t";

    for (unsigned int j = 0; j < 8; j++)
        logData << muscles[j]->getForce() << "\t";
